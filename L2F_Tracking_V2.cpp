    ...
    const cv::Size& inputSize = cv::Size(640, 640))
{
    ...
    // Get input name (modern API)

    auto inputName = session.GetInputNameAllocated(0, allocator);
    inputNameStrings.push_back(std::string(inputName.get()));
    inputNames.push_back(inputNameStrings.back().c_str());

    // Get output name (modern API)
    auto outputName = session.GetOutputNameAllocated(0, allocator);
    outputNameStrings.push_back(std::string(outputName.get()));
